...
afterAll(async () => {
  await prisma.user.deleteMany({ where: { email: { contains: "test+role@" } } }).catch(() => {});
...
test("roles: aprendiz cannot create, mentor can create, mentor cannot approve, master can approve", async () => {
  ...
  await app.inject({ method: "POST", url: "/auth/register", payload: { email: emailApr, password: "password123", role: "APRENDIZ" } });
  const loginApr = await app.inject({ method: "POST", url: "/auth/login", payload: { email: emailApr, password: "password123" } });
  ...
  await app.inject({ method: "POST", url: "/auth/register", payload: { email: emailMent, password: "password123", role: "MENTOR" } });
  const loginMent = await app.inject({ method: "POST", url: "/auth/login", payload: { email: emailMent, password: "password123" } });
  ...
  await prisma.user.create({ data: { email: emailMaster, name: emailMaster, passwordHash: passHash, roleId: masterRole!.id } });
  const loginMaster = await app.inject({ method: "POST", url: "/auth/login", payload: { email: emailMaster, password: "password123" } });
  ...
  const aprCreate = await app.inject({ method: "POST", url: "/content/create", headers: { authorization: `Bearer ${aprToken}` } });
  expect(aprCreate.statusCode).toBe(403);

  // mentor can create -> 201
  const mentCreate = await app.inject({ method: "POST", url: "/content/create", headers: { authorization: `Bearer ${mentToken}` } });
  expect(mentCreate.statusCode).toBe(201);

  // mentor cannot approve -> 403
  const mentApprove = await app.inject({ method: "POST", url: "/content/approve", headers: { authorization: `Bearer ${mentToken}` } });
  expect(mentApprove.statusCode).toBe(403);

  // master can approve -> 200
  const masterApprove = await app.inject({ method: "POST", url: "/content/approve", headers: { authorization: `Bearer ${masterToken}` } });
