 ...
 */ function assignDefaultsAndValidate(dir, userConfig, silent, phase) {
    ...
    if (process.env.__NEXT_TEST_MODE && process.env.IS_TURBOPACK_TEST && result.deploymentId && process.env.__NEXT_IMMUTABLE_ASSET_TOKEN) {
        result.experimental.immutableAssetToken = process.env.__NEXT_IMMUTABLE_ASSET_TOKEN;
    }
...
export default async function loadConfig(phase, dir, { customConfig, rawConfig, silent = true, reportExperimentalFeatures, reactProductionProfiling, debugPrerender, bundler } = {}) {
    ...
    if (path == null ? void 0 : path.length) {
        ...
        if (phase === PHASE_PRODUCTION_BUILD && bundler !== Bundler.Turbopack && ((_userConfig_experimental = userConfig.experimental) == null ? void 0 : _userConfig_experimental.immutableAssetToken)) {
